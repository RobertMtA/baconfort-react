...
class GalleryEventManager {
  ...
  setupCrossTabCommunication() {
    ...
    window.addEventListener('storage', (event) => {
      if (event.key === 'baconfort-gallery-update') {
        ...
        if (propertyId) {
          console.log(`🔄 CROSS-TAB EVENT: Recibido cambio para ${propertyId}`);
    ...
    if (this.listeners.has(propertyId)) {
      this.listeners.get(propertyId).forEach(callback => {
        try {
          ...
          console.error('Error ejecutando callback de galería:', error);
        }
      });
    }
  }
  ...
  notifyGalleryChanged(propertyId) {
    console.log(`📢 GALLERY EVENT: Notificando cambios para ${propertyId}`);
