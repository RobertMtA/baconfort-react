...
const NewPropertyCreator = () => {
  ...
  const handleGalleryImagesChange = (e) => {
    ...
    console.log('🔍 Debug: Archivos:', files.map(f => f.name));
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      if (file.size > 5 * 1024 * 1024) {
        invalidFiles.push(`${file.name} es muy grande (máximo 5MB)`);
        continue;
      }

      if (!file.type.startsWith('image/')) {
        invalidFiles.push(`${file.name} no es un archivo de imagen válido`);
        continue;
      }
    ...
    // Si no hay archivos válidos, mostrar errores
    if (validFiles.length === 0) {
      setMessage(`❌ No se seleccionaron imágenes válidas. Errores: ${invalidFiles.join(', ')}`);
      return;
    }
    
    // Procesar archivos válidos
    for (let i = 0; i < validFiles.length; i++) {
      const { file, originalIndex } = validFiles[i];
      ...
      reader.onload = (e) => {
        ...
        // Cuando todas las imágenes hayan sido cargadas, actualizar el estado
        if (loadedCount === validFiles.length) {
          // Ordenar previews por índice para mantener el orden original
          previews.sort((a, b) => a.index - b.index);
          setGalleryPreviews(previews);
          setGalleryImages(validFiles.map(vf => vf.file));
          
          // Mensaje de éxito con advertencias si las hay
          let successMessage = `✅ ${validFiles.length} imagen(es) cargada(s) correctamente`;
          if (invalidFiles.length > 0) {
            successMessage += `. Archivos omitidos: ${invalidFiles.length}`;
          }
          setMessage(successMessage);
          
          console.log('✅ Debug: Imágenes cargadas exitosamente:', validFiles.length);
          if (invalidFiles.length > 0) {
            console.warn('⚠️ Debug: Archivos omitidos:', invalidFiles);
          }
        }
      };
      ...
      reader.onerror = (error) => {
        ...
        // Continuar con el procesamiento aunque falle un archivo
        if (loadedCount === validFiles.length) {
          setGalleryPreviews(previews);
          setGalleryImages(validFiles.map(vf => vf.file));
          setMessage(`⚠️ ${previews.length} imagen(es) cargada(s) con algunos errores`);
        }
      };
  ...
  // Función optimizada para crear thumbnails pequeños (para evitar quota exceeded)
  const createThumbnail = (file, maxWidth = 200, quality = 0.3) => {
    return new Promise((resolve, reject) => {
  ...
  const handleCreateProperty = async () => {
    try {
      ...
      if (coverImage) {
        try {
          // Usar thumbnail para la imagen de portada (más pequeño)
          coverImageBase64 = await createThumbnail(coverImage, 400, 0.7);
          console.log('✅ Thumbnail de portada procesado');
        } catch (error) {
      ...
      if (galleryImages.length > 0) {
        ...
        // Procesar solo las primeras 4 imágenes para evitar problemas de storage
        const maxImages = Math.min(galleryImages.length, 4);
        
        for (let i = 0; i < maxImages; i++) {
          try {
            // Usar thumbnail pequeño para galería
            const thumbnailBase64 = await createThumbnail(galleryImages[i], 300, 0.5);
            galleryImagesBase64.push(thumbnailBase64);
            setMessage(`Procesando imagen ${i + 1} de ${maxImages}...`);
          } catch (error) {
        ...
        if (galleryImages.length > 4) {
          console.log(`⚠️ Solo se procesaron las primeras 4 imágenes de ${galleryImages.length} para evitar problemas de storage`);
        }
        
        console.log(`✅ ${galleryImagesBase64.length} thumbnails de galería procesados`);
      }
              ...
              <div className="card-body">
                <div className="row">
                  ...
                  <div className="col-md-6 mb-4">
                    ...
                    />
                    <small className="form-text text-muted">
                      <strong>💡 Instrucciones:</strong> Mantén presionado <kbd>Ctrl</kbd> (Windows) o <kbd>Cmd</kbd> (Mac) para seleccionar múltiples imágenes.
                      <br />
