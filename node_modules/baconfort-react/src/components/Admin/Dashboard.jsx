...
function Dashboard({ onEditProperty }) {
  ...
  const [activeSection, setActiveSection] = useState('overview'); // Estado para controlar qué sección mostrar

  // Dashboard inicializado

  // NO cargar desde backend - AdminProvider ya lo hace automáticamente
  ...
  // Calcular zonas cubiertas dinámicamente
  const uniqueZones = new Set();
  propertyList.forEach(property => {
    if (property.address) {
      // Extraer la zona/barrio de la dirección
  ...
  const viewProperty = (propertyId) => {
    // Mapeo de IDs a rutas estáticas (propiedades con páginas dedicadas)
    ...
    // Verificar si es una propiedad con página estática
    ...
    // Para propiedades dinámicas, usar la ruta genérica
    const dynamicRoute = `/departamentos/${propertyId}`;
    window.open(dynamicRoute, '_blank');
  };

  // Función para BLOQUEAR propiedades
  const blockProperty = async (propertyId) => {
    const property = properties[propertyId];
    
    if (property?.isBlocked) {
      alert('La propiedad ya está bloqueada');
      return;
    }
    
    try {
      console.log(`🔒 Bloqueando propiedad ${propertyId}`);
      ...
      console.log(`✅ Propiedad ${propertyId} bloqueada exitosamente`);
    } catch (error) {
      console.error(`❌ Error al bloquear ${propertyId}:`, error);
      alert(`Error al bloquear la propiedad: ${error.message}`);
    }
  };

  // Función para DESBLOQUEAR propiedades
  const unblockProperty = async (propertyId) => {
    const property = properties[propertyId];
    
    if (!property?.isBlocked) {
      alert('La propiedad ya está desbloqueada');
      return;
    }
  ...
  // Función para ELIMINAR propiedades - Simplificada
  const handleDeleteProperty = async (propertyId) => {
    const property = properties[propertyId];
    const propertyTitle = property?.title || 'esta propiedad';
    
    // Verificar si la propiedad tiene archivos/páginas estáticas
    const hasStaticPage = ['moldes-1680', 'santa-fe-3770', 'dorrego-1548', 'convencion-1994', 'ugarteche-2824'].includes(propertyId);
    
    if (hasStaticPage) {
      alert('⚠️ No se puede eliminar esta propiedad porque tiene archivos y página estática asociada.');
      return;
    }
    
    // Confirmación simple
    const confirmDelete = window.confirm(`¿Eliminar "${propertyTitle}" permanentemente?`);
    ...
    try {
      console.log('🗑️ Dashboard: Eliminando propiedad:', propertyId);
      
      // Llamar a la función de eliminación del contexto
      await deleteProperty(propertyId);
      
      console.log('✅ Dashboard: Propiedad eliminada exitosamente');
      alert('✅ Propiedad eliminada exitosamente');
      
    } catch (error) {
      console.error('❌ Dashboard: Error al eliminar propiedad:', error);
  ...
  return (
    <div className="dashboard" style={{backgroundColor: 'transparent !important'}}>      
      ...
      {/* Nueva sección para gestión de propiedades nuevas */}
      <div className="new-property-tools" style={{ marginBottom: '2rem' }}>
        <div className="card border-primary">
          <div className="card-header bg-primary text-white">
            <h5 className="mb-0">
              <i className="fas fa-plus-circle me-2"></i>
              Herramientas para Propiedades Nuevas
            </h5>
          </div>
              ...
              <div className="col-md-6">
                <div className="d-grid">
                  <button 
                    ...
                    onClick={() => window.location.href = '/admin/new-property'}
                  >
                ...
                <small className="text-muted mt-2 d-block">
                  Crea propiedades completamente nuevas sin afectar las existentes
                </small>
              </div>
              <div className="col-md-6">
                <div className="d-grid">
                  <button 
                    ...
                    onClick={() => window.location.href = '/admin/manage-new-properties'}
                  >
                    <i className="fas fa-cogs me-2"></i>
                    Gestionar Propiedades Nuevas
                  </button>
                </div>
                <small className="text-muted mt-2 d-block">
                  Administra solo las propiedades que creaste (no las existentes)
                </small>
              </div>
        ...
        <div className="dashboard-section">
          ...
          <div className="properties-overview">
            ...
            <div className="properties-grid">
              {propertyList
                .filter(property => property && property.id) // Filtrar propiedades válidas
                .map(property => (
                <div key={property.id} className={`property-card ${property.isBlocked ? 'blocked' : ''}`}>
                  ...
                  <div className="property-info">
                    <h4>{property.title || 'Sin título'}</h4>
                    ...
                    <div className="property-actions">
                      <div className="main-actions">
                        ...
                        {/* Botón de eliminación solo para propiedades sin archivos */}
        ...
        <div className="dashboard-section">
          ...
          <div className="calendars-dashboard-section">
            <h3>
              ...
              Calendarios de Ocupación por Propiedad
            </h3>
      ...
      {/* Sección de Debug removida para producción */}
    </div>
  );
}
