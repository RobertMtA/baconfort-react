...
// Esquema exacto del modelo Gallery (debe coincidir con el backend)
...
async function fixGalleries() {
  try {
    console.log('🧹 Limpiando galerías incorrectas...');
    
    // Eliminar registros con formatos incorretos
    const deleteResult = await Gallery.deleteMany({
      $or: [
        { propertyId: { $in: ['santafe3770', 'dorrego1548', 'convencion1994', 'ugarteche2824'] } },
        { isMainImage: { $exists: true } }
      ]
    });
    console.log(`🗑️  Eliminados ${deleteResult.deletedCount} registros incorrectos`);
    
    console.log('📁 Recreando galerías con formato correcto...');
    ...
    for (const [propertyId, config] of Object.entries(properties)) {
      ...
      for (let i = 0; i < config.images.length; i++) {
        ...
        // Crear registro en la base de datos con el esquema correcto
        const galleryItem = new Gallery({
          ...
          title: '',
          description: '',
          altText: filename.replace('.jpg', '').replace('img-', ''),
    ...
    console.log('\n🎉 ¡Galerías arregladas correctamente!');
    
    // Mostrar resumen
    for (const propertyId of Object.keys(properties)) {
      const count = await Gallery.countDocuments({ propertyId });
      const mainImage = await Gallery.findOne({ propertyId, isMain: true });
      console.log(`📊 ${propertyId}: ${count} imágenes, imagen principal: ${mainImage?.filename || 'NINGUNA'}`);
    }
